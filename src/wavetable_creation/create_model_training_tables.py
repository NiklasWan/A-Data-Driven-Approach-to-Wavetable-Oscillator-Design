...
def extract_cycles(audio_dict, smoothing, max_tables):
    ...
    for i, name in enumerate(cycles['name']):

        if len(name) < max_len:
            idx = i
            max_len = len(name)

    out_dict = dict(name=[], data=[])
    if idx < max_tables:
        for i, names in enumerate(cycles['name']):

            out_dict['data'].append(cycles['data'][i][:max_len])
            out_dict['name'].append(names[:max_len])
...
def main():
    ...
    save_to_disk(table_dict, output_path)
