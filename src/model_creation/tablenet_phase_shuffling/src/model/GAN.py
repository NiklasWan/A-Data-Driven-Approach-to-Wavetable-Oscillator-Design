...
class GAN(object):
    ...
    def fit(self, data_generator, niter=20000, nbatch=64, k=5, opt=None, save_dir='../../train/', save_iter=20):
        if opt == None:
            opt = Adam(lr=0.0001, beta_1=0.5, beta_2=0.9)
        ...
        self.gan.save(os.path.join('../../train/model/', 'trained_model_phase_shuffling.h5'))
        self.generator.save(os.path.join('../../train/model/', 'trained_generator_phase_shuffling.h5'))
        self.discriminator.save(os.path.join('../../train/model/', 'trained_discriminator_phase_shuffling.h5'))
